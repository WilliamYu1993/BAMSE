...
######################### Training data #########################
Train_Air_Noisy_lists = get_filepaths("/mnt/md2/user_khhung/bone_conduct/wavfile/Train/Noisy")#training noisy set
Train_Air_Clean_paths = "/mnt/md2/user_khhung/bone_conduct/wavfile/Train/Air/"

random.shuffle(Train_Air_Noisy_lists)

idx = int(len(Train_Air_Noisy_lists)*0.95)

Train_lists = Train_Air_Noisy_lists[0:idx]
Num_traindata = len(Train_lists)
Valid_lists = Train_Air_Noisy_lists[idx:]


steps_per_epoch = (Num_traindata)//batch_size
######################### Test_set #########################
Test_lists = get_filepaths("/mnt/md2/user_khhung/bone_conduct/wavfile/Test/Noisy")#[0:25] #testing noisy set
...
with open('FCN_a2a_V2.json','w') as f:    # save the model
    f.write(model.to_json()) 
checkpointer = ModelCheckpoint(filepath='FCN_a2a_V2.hdf5', verbose=1, save_best_only=True, mode='min')  
#pdb.set_trace()
'''
...
g1 = data_generator(Train_lists, Train_Air_Clean_paths, shuffle = "True")
g2 = valid_generator(Valid_lists, Train_Air_Clean_paths, shuffle = "False")                					
...
print('load model')
MdNamePath='FCN_a2a_V1' #the model path
...
print('testing...')
#pdb.set_trace()
for path in Test_lists: # Ex: /mnt/Nas/Corpus/TMHINT/Testing/Noisy/car_noise_idle_noise_60_mph/b4/1dB/TMHINT_12_10.wav
    if path.split("/")[-1][-4:] == ".wav":
        ...
        creatdir(os.path.join("FCN_a2a_MSE_wav_V2", noise, speaker, dB))
        librosa.output.write_wav(os.path.join("FCN_a2a_MSE_wav_V2", noise, speaker, dB, wave_name), enhanced, 16000)
tEnd = time.time()
...
plt.grid(True)
plt.show()
plt.savefig('FCN_a2a_Learning_curve_V2.png', dpi=150)
