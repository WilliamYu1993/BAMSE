...
def joint():
    ...
    bone = Convolution1D(1, 255, border_mode='same', input_shape=ishape)(input_bone)
    input_air = Input(shape=(None,1))
    air = Convolution1D(1, 55, border_mode='same', input_shape=ishape)(input_air)
    avg = Merge()([bone, air])
...
model_bone.add(Convolution1D(1, 255, border_mode='same', input_shape=(None,1)))
model_bone.add(BatchNormalization(mode=2,axis=-1))

model_air = Sequential()
model_air.add(Convolution1D(1, 55, border_mode='same', input_shape=(None,1)))
model_air.add(BatchNormalization(mode=2,axis=-1))
model_air.add(LeakyReLU())
'''
model = Sequential()
#model.add(Merge([model_bone, model_air], mode='concat', concat_axis=-1))

model.add(Convolution1D(30, 115,  border_mode='same',input_shape=(None,2)))
...
model.add(Convolution1D(1, 115,  border_mode='same'))
...
with open('FCN_ab_V1.json','w') as f:    # save the model
    f.write(model.to_json()) 
checkpointer = ModelCheckpoint(filepath='FCN_ab_V1.hdf5', verbose=1, save_best_only=True, mode='min')  
#pdb.set_trace()
'''
...
print('load model')
MdNamePath='FCN_ab_V1' #the model path
...
for path in Test_Noisy_lists: # Ex: /mnt/Nas/Corpus/TMHINT/Testing/Noisy/car_noise_idle_noise_60_mph/b4/1dB/TMHINT_12_10.wav
    if path.split("/")[-1][-4:] == ".wav":
        ...
        creatdir(os.path.join("FCN_ab_MSE_wav_V1", noise, speaker, dB))
        librosa.output.write_wav(os.path.join("FCN_ab_MSE_wav_V1", noise, speaker, dB, wave_name), enhanced, 16000)
tEnd = time.time()
...
plt.grid(True)
plt.show()
plt.savefig('FCN_ab_Learning_curve_V1.png', dpi=150)
